...
/***/ function(module, exports, __webpack_require__) {
	...
	var _platenum = __webpack_require__(243);

	var _platenum2 = _interopRequireDefault(_platenum);
	...
	var STYLE_MAP = {
	    '0': '',
	    '1': '',
	    '2': _platenum2.default.three,
	    '3': _platenum2.default.four
	};
	var KEYBOARD_MAP = {
	    '0': '',
	    '1': '',
	    '2': '',
	    '3': _platenum2.default.last
	};
...
/***/ function(module, exports) {
	...
	module.exports = {"wraper":"platenum__wraper___MW2S_","active":"platenum__active___7jA0E","list":"platenum__list___314P8","row":"platenum__row___vL6ew","clearfix":"platenum__clearfix___3aoU0","item":"platenum__item___1lYS1","three":"platenum__three___1jjaX","four":"platenum__four___2yvcV","last":"platenum__last___eit7z","delete":"platenum__delete___3kqrL"};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PROVINCE = [['京', '津', '冀', '鲁', '晋', '蒙', '辽', '吉', '黑', '沪'], ['苏', '浙', '皖', '闽', '赣', '豫', '鄂', '湘', '粤', '桂'], ['渝', '川', '贵', '云', '藏', '陕', '甘', '青'], ['琼', '新', '港', '澳', '台', '宁']];
	var KEYBOARD = [['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'], ['港', 'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'O', 'P'], ['澳', 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L'], ['Z', 'X', 'C', 'V', 'B', 'N', 'M']];

	function PlateNumber(opts) {
	    if (!instance) {
	        this.opts = $.extend({}, __DEFAULT__, opts);
	        this.id = 'platenum-' + (new Date() - 0);
	        this.value = [this.opts.defaultProvince, this.opts.defaultCity];
	        _render.call(this);
	...
	PlateNumber.create = function (opts) {
	    if (!instance) {
	        instance = new PlateNumber(opts);
	    }
	    return instance;
	};
	...
	function _render() {
	    var html = (0, _index2.default)({
	        ...
	        province: PROVINCE,
	        keyboard: KEYBOARD,
	        style: _platenum2.default
	    });
	    $('body').append(html);
	    //dom绑定
	    this.panel = $('#' + this.id);
	    var keyboards = this.panel.find('.' + _platenum2.default.list);

	    this.provincePanel = keyboards.eq(0);
	    this.keyboardPanel = keyboards.eq(1);
	}

	function _bindEvent() {
	    var that = this;
	    this.panel.on('touchstart', function (e) {
	        e.stopPropagation();
	    });
	    this.panel.delegate('.' + _platenum2.default.item, 'touchstart', function (e) {
	        e.stopPropagation();
	        var type = $(this).parents('.' + _platenum2.default.list).data('type');
	        ...
	        //触发更改事件
	        that.opts.onClick.call(that, index - 1, value, that.value.join(''));
	        if (that.value.length == 7 && that.value.join('').length == 7) {
	            that.opts.onComplete.call(that, that.value.join(''));
	        }
	    });
	    //删除按钮
	    this.panel.delegate('.' + _platenum2.default.delete, 'touchstart', function (e) {
	        that.opts.onDelete(index);
	        that.value[index] = '';
	    });
	}

	PlateNumber.prototype = {
	    /**
	     * 显示车牌输入软键盘
	     * type=1,显示省份
	     * type=2,显示常规键盘
	     */
	    show: function show(type, i) {
	        this.panel.addClass(_platenum2.default.active);
	        if (type == 1) {
	            index = 0;
	            this.keyboardPanel.removeClass(_platenum2.default.active);
	            this.provincePanel.addClass(_platenum2.default.active);
	        } else {
	            index = i;
	            this.provincePanel.removeClass(_platenum2.default.active);
	            this.keyboardPanel.addClass(_platenum2.default.active);
	        }
	    },
	    hide: function hide() {
	        this.panel.removeClass(_platenum2.default.active);
	        this.provincePanel.removeClass(_platenum2.default.active);
	        this.keyboardPanel.removeClass(_platenum2.default.active);
	    },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _plateNumber = __webpack_require__(249);

	var _plateNumber2 = _interopRequireDefault(_plateNumber);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	__webpack_require__(290);
	//require('../../js/registerNext/registerNext.js');

	_factory2.default.create('account.addcar', function () {
	    var keyboard;
	    /**
	        *  初始化虚拟键盘
	    ...
	    function _initKeyBoard() {
	        var that = this;
	        keyboard = _plateNumber2.default.create({
	            defaultProvince: '鲁',
	            defaultCity: 'B',
	            onComplete: function onComplete(plateno) {
	                //  var color = $('.car-list').find('.car-item.active').data('value');
	                //  _checkPlateNo.call(that,color,plateno);
	            },
	            onClick: function onClick(index, value, plateno) {
	                var $values = $('.code-list').find('.code-item');
	                $values.eq(index).html(value);
	                $values.eq(index + 1).addClass('active').siblings('.active').removeClass('active');
	                //更新列表
	                $('.car-list').find('.car-item').each(function (i, item) {
	                    $(item).find('.code-item').eq(index).html(value);
	                });
	            },
	            onDelete: function onDelete(index) {
	                console.log(index);
	                var $prevItem = $('.code-list').find('.code-item').eq(index - 1);
	                var $item = $('.code-list').find('.code-item').eq(index);
	                ...
	                if (index == 1) {
	                    ...
	                    if ($item.html()) {
	                        $('.code-list').find('.code-item').eq(index).html('');
	                        $('.car-list').find('.car-item').each(function (i, item) {
	                            $(item).find('.code-item').eq(index).html(9);
	                        });
	                    } else {
	                        $item.removeClass('active');
	                        $prevItem.html('').addClass('active');
	                        $('.car-list').find('.car-item').each(function (i, item) {
	                            $(item).find('.code-item').eq(index - 1).html(9);
	                        });
	                        keyboard.delete();
	                    }
	                }
	    ...
	    function _bindEvent() {
	        $('.code-list').delegate('.code-item', 'touchstart', function (e) {
	            e.stopPropagation();
	            var type = this.dataset.type - 0;
	            var index = this.dataset.num - 0;
	            $(this).addClass('active').siblings('.active').removeClass('active');
	            //省份
	            if (type == 1) {
	                keyboard.show(1);
	            } else {
	                keyboard.show(2, index);
	            }
	        });

	        $('.car-list').delegate('li.car-item', 'click', function () {
	            var $this = $(this);
	            $this.siblings('.active').removeClass('active');
	            $this.addClass('active');
	            var color = $this.data('value');
	            //_checkPlateNo.call(that,color,keyboard.getValue());
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\""
	    ...
	    + "\">\r\n    <ul class=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.style : depth0)) != null ? stack1.list : stack1), depth0))
	    + "\" data-type=\"1\">\r\n"
	    + ((stack1 = container.invokePartial(__webpack_require__(315),depth0,{"name":"province","data":data,"indent":"        ","helpers":helpers,"partials":partials,"decorators":container.decorators})) != null ? stack1 : "")
	    + "    </ul>\r\n    <ul class=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.style : depth0)) != null ? stack1.list : stack1), depth0))
	    + "\"  data-type=\"2\">\r\n"
	    + ((stack1 = container.invokePartial(__webpack_require__(314),depth0,{"name":"keyboard","data":data,"indent":"        ","helpers":helpers,"partials":partials,"decorators":container.decorators})) != null ? stack1 : "")
	    + "    </ul>\r\n</div>";
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams,depths) {
	  ...
	  return "    <li class=\""
	    + alias2(alias1(((stack1 = (depths[1] != null ? depths[1].style : depths[1])) != null ? stack1.row : stack1), depth0))
	    + " "
	    + alias2(__default(__webpack_require__(240)).call(alias3,(data && data.index),2,{"name":"getStyle","hash":{},"data":data}))
	    + " "
	    + alias2(alias1(((stack1 = (depths[1] != null ? depths[1].style : depths[1])) != null ? stack1.clearfix : stack1), depth0))
	    + "\">\r\n"
	    + ((stack1 = helpers.each.call(alias3,depth0,{"name":"each","hash":{},"fn":container.program(2, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
	    + ((stack1 = helpers["if"].call(alias3,(data && data.last),{"name":"if","hash":{},"fn":container.program(4, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
	    + "    </li>\r\n";
	},"2":function(container,depth0,helpers,partials,data,blockParams,depths) {
	    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

	  return "            <div class=\""
	    + alias2(alias1(((stack1 = (depths[2] != null ? depths[2].style : depths[2])) != null ? stack1.item : stack1), depth0))
	    + "\" data-value=\""
	  ...
	  return "            <span class=\""
	    + container.escapeExpression(container.lambda(((stack1 = (depths[1] != null ? depths[1].style : depths[1])) != null ? stack1["delete"] : stack1), depth0))
	    + "\"></span>\r\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams,depths) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.keyboard : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "");
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams,depths) {
	  ...
	  return "    <li class=\""
	    + alias2(alias1(((stack1 = (depths[1] != null ? depths[1].style : depths[1])) != null ? stack1.row : stack1), depth0))
	    + " "
	    + alias2(__default(__webpack_require__(240)).call(alias3,(data && data.index),1,{"name":"getStyle","hash":{},"data":data}))
	    + " "
	    + alias2(alias1(((stack1 = (depths[1] != null ? depths[1].style : depths[1])) != null ? stack1.clearfix : stack1), depth0))
	    + "\">\r\n"
	    + ((stack1 = helpers.each.call(alias3,depth0,{"name":"each","hash":{},"fn":container.program(2, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "")
	  ...
	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.province : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0, blockParams, depths),"inverse":container.noop,"data":data})) != null ? stack1 : "");
	},"useData":true,"useDepths":true});
