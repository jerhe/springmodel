...
/***/ function(module, exports, __webpack_require__) {
	...
	//引入模板
	var tplRender = __webpack_require__(131);
...
/***/ function(module, exports, __webpack_require__) {
	...
	_index2.default.create('fee.scan', function () {

		var plateNo; //车牌号
		function _getAjax(api, url) {
			(0, _ajax2.default)({
				url: api,
				data: {
					plateNo: plateNo
					...
					if (err) {
						...
						_setInPic(res); //设置车场入场图片
					}
				}
			});
		}
		function _setInPic(data) {
			$('img').attr('src', data.pic);
		}
		function _bindEvent() {
			var api = this.opts.api;
			var url = this.opts.confirmURL;
			var appKey = this.opts.appKey;
			var voucherCode = this.opts.voucherCode;
			var length; //输入长度
			$('#confirm').on('click', function () {
				if (url.indexOf('?') > 0) {
					location.href = url + "&appKey=" + appKey + "&voucherCode=" + voucherCode + "&plateNo=" + plateNo;
				} else {
					location.href = url + "?appKey=" + appKey + "&voucherCode=" + voucherCode + "&plateNo=" + plateNo;
				}
			});
			$('#carPlate').on('keyup', function () {
				//输入框触发事件
				length = $('#carPlate').val().toString().length;
				if (length == 7 || length == 8) {
					plateNo = $('#carPlate').val();
					_getAjax(api, url);
				}
			});
		}
