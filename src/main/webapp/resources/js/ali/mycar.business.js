	...
	onDelete:function(){
		...
		plugin.Dialog.confirm('确定要删除该车辆吗',function(item){
			if(item.value === 1){
				...
				$.ajax({
					type : 'post',
					url : contextPath + '/ali/d/pc/deleteCar',
					data : {"perVehicleid" : vehicleId,"aliAppId":aliAppId,"aliUserId":aliUserId},
					dataType : "json",
					success : function(result) {
						if (result.isOk) {
							plugin.Toast.show('info','删除成功');
							location.href = contextPath + "/ali/j/pc/goMyCar?aliAppId="+aliAppId+"&aliUserId="+aliUserId;
						} else {
...
$(".yan").click(function(){
	...
	if(cagrade != 1){
		window.location.href = contextPath+"/ali/j/pc/goVerifycar?id="+id+"&plateno="+plateno+"&sessionKey="+sessionKey+"&aliAppId="+aliAppId+"&aliUserId="+aliUserId;
	}else{
...
$("#add").click(function(){
	if(countCar < 10){
		window.location.href =  contextPath+"/ali/j/pc/goAddCar?sessionKey="+sessionKey+"&aliAppId="+aliAppId+"&aliUserId="+aliUserId;
	}else{
...
//	if(cagrade == 1){
		if(isEtc == "0"){//开启
			url = contextPath + '/ali/d/pc/setVehicleOpenEtc';
			$.ajax({
				type : 'post',
				url : url,
				data : {
			...
			url = contextPath + '/ali/d/pc/setVehicleCloseEtc';
			$.ajax({
				type : 'post',
				url : url,
				data : {
